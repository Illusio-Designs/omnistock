# Kartriq Integration Setup Guide

This guide lists every channel Kartriq supports, where to register the developer app, what credentials you need, and where to paste them.
...
| **Per-seller API key** | Each seller generates their own keys in the provider's dashboard and pastes them into Kartriq. | Shiprocket, Delhivery, WooCommerce, Meesho, Nykaa |
...
You as the founder register **one** Amazon SP-API "Public App" for the entire Kartriq platform. Every tenant uses *that one app* — they never register their own with Amazon. Each tenant's authorization produces a per-tenant `refreshToken` that's stored encrypted, scoped to their tenant only.
...
| Paste credentials in Kartriq | `/admin/settings` → Amazon SP-API tab | Saved as `amazon.clientId`, `amazon.clientSecret` (encrypted) |
...
| Approve the Kartriq app | Amazon's screen (NOT Kartriq) | Returns to Kartriq with an authorization code |
| Kartriq exchanges the code for a `refreshToken` | Automatic, via the callback URL above | Stored encrypted on the tenant's `channel` row alongside their `sellerId` and `region` |

When Kartriq makes an API call for **tenant X**, the adapter assembles:
```
...
## Quick reference — Kartriq admin paths
...
Each seller goes to **Channels → Amazon Smart Biz → Connect** → picks their region → clicks **Authorize with Amazon Smart Biz**. A popup opens Seller Central, they approve, and the popup auto-closes. Kartriq stores per-seller `{ sellerId, refreshToken, region }` encrypted.
...
> **Status**: Shopify OAuth flow in Kartriq is **pending** — the settings slot exists but the `/oauth/shopify/start` + `/callback` routes are not yet wired. Sellers currently use the per-store paste form.
...
Leave all three blank during development — Kartriq runs in **stub mode** and the billing flow works without real charges.
...
| `smtp.from` | `Kartriq <no-reply@yourdomain.com>` |

Leave blank → Kartriq logs every email to the console (dev stub mode).
...
Each tenant logs into their Kartriq dashboard and connects channels they actually sell on. There are two flavors:
...
- **Paste-form channels** (everything in §4 below) — tenant generates their own keys in the channel's seller portal and pastes them into Kartriq.
...
Use this when you have your own platform that should POST orders to Kartriq.
...
Every seller's channel has its own public webhook endpoint. Paste the URL from Kartriq → Channel detail → Connect modal into the external system's webhook settings.
...
| Razorpay → Kartriq payments | `POST /api/v1/payments/webhook` |
...
SMTP_FROM=Kartriq <no-reply@example.com>
```
