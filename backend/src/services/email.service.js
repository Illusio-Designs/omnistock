...
async function getTransport() {
  ...
  if (cached.transporter && cached.fromKey === cacheKey) {
    return { transporter: cached.transporter, from: from || 'Omnistock <no-reply@omnistock.com>' };
  }
  ...
  try {
    ...
    return { transporter, from: from || 'Omnistock <no-reply@omnistock.com>' };
  } catch {
...
async function send({ to, subject, html, text }) {
  ...
  if (!transporter) {
    console.log('\n[email:stub] ---------------------------------');
    console.log(`  from:    ${from || 'Omnistock'}`);
    console.log(`  to:      ${to}`);
...
<div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px;">
  ...
  <div style="color:#94a3b8;font-size:11px;border-top:1px solid #e2e8f0;padding-top:12px;">
    You're receiving this because you signed up to Omnistock.
  </div>
</div>`;

const sendWelcome = ({ to, name, businessName }) => send({
  to,
  subject: `Welcome to Omnistock, ${name}!`,
  html: wrap(`
...
const sendTrialEndingSoon = ({ to, name, daysLeft }) => send({
  to,
  subject: `Your Omnistock trial ends in ${daysLeft} days`,
  html: wrap(`
